function file_list = write_wav_file_SM(file_name,file_list,file_prefix,...
                                    songfilt_args)
...
for jj = 1:nfiles
    ...
    directory = direc{end};
    TA = strcmp (directory,'wavfiles_calls_Always');
    TO = strcmp (directory,'wavfiles_calls_Once');
    TT = TA + TO;
    
    [y,Fin,bits] = wavread(infilename);
    
    if TT~=1 %this step has already been done on calls in...
        %wavfiles_calls by running Process_wav_files_call.m
        
        %if y is a stereo file->converts to mono
        if length(y(1,:))>1
            y = (y(:,1)+ y(:,2))/2;
        end
        y = simplefilt(y,Fin,songfilt_args.f_low,songfilt_args.f_high,...
                 songfilt_args.db_att); % !!!! SM changed the function songfilt to simplefilt on March16, 2012!
