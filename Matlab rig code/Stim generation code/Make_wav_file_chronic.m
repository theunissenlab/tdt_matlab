...
% The STRF1 files are found in directory 'P1'
Stims(2).dir = fullfile(pwd,'P1');
Stims(2).name = 'P1';
Stims(2).randomtype = 1;
Stims(2).stimset = 1;

% The STRF2 files are found in directory 'P2'
Stims(3).dir = fullfile(pwd,'P2');
Stims(3).name = 'P2';
Stims(3).randomtype = 1;
Stims(3).stimset = 1;

% The STRF3 files are found in directory 'P3'
...
for jj = 1:nStimSets
    for kk=1:Stims(jj).nfiles
        ...
        if length(y(1,:))>1
            ...
            fprintf(1,'the stim #%d from %s was stereo converted to mono\n', kk, Stims(jj).dir);
        end
        pts = [pts,size(y,1)];
